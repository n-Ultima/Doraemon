...
namespace Doraemon.Data.Events.MessageReceivedHandlers
{
    public class AutoModeration
    {
        ...
        public async Task CheckForSpamAsync(SocketMessage arg)
        {
            ...
            if (message.Content.Count() > 1200)
            {
                ...
                if (!context.User.IsStaff())
                {
                    ...
                    var warnLog = context.Guild.Channels.FirstOrDefault(x => x.Name == "doraemon-logs") as SocketTextChannel;
                    if (warnLog is null)
                    {
                    ...
                    await _infractionService.CreateInfractionAsync(message.Author.Id, autoModId, context.Guild.Id, "Warn", "Spamming characters in a message");
                    return;
                }
            }
            if (message.Content.Split("\n").Length > 6)
            {
                if (!context.User.IsStaff())
                {
                    ...
                    await _infractionService.CreateInfractionAsync(message.Author.Id, autoModId, context.Guild.Id, "Warn", "Spamming lines in a message.");
                }
            }
        }
        public async Task CheckForBlacklistedAttachmentTypesAsync(SocketMessage arg)
        {
            ...
            await _infractionService.CreateInfractionAsync(message.Author.Id, selfUser.Id, guild.Id, "Warn", "Posting suspicious files.");
        }
